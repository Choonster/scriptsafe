...
function loadOptions() {
      ...
      if (blockedarr[i][2] == 'NOSCRIPT' || blockedarr[i][2] == 'WEBBUG') {
        $('#blocked > table > tbody').append(
          '<tr rel="' +
            itemdomainfriendly +
            '"><td>' +
            padZeros(entryTime.getHours()) +
            ':' +
            padZeros(entryTime.getMinutes()) +
            ':' +
            padZeros(entryTime.getSeconds()) +
            '</td><td title="' +
            blockedarr[i][1].replace(/"/g, "'") +
            '">' +
            truncate(blockedarr[i][1]) +
            '</td><td>' +
            blockedarr[i][2] +
            '</td><td title="' +
            blockedarr[i][4] +
            '">' +
            truncate(blockedarr[i][4]) +
            '</td><td class="text-right" data-domain="' +
            itemdomain +
            '">&nbsp;</td>',
        );
      } else if (
        ...
        $('#blocked > table > tbody').append(
          '<tr rel="' +
            ...
            '">' +
            truncate(blockedarr[i][4]) +
            '</td><td class="text-right choices" data-domain="' +
            itemdomain +
            '" rel="' +
            blockedarr[i][3] +
            '"><span class="box box2 x_blacklist selected" rel="1" title="' +
            unwantedType +
            '">' +
            unwantedType +
            '</span></td>',
        ...
        $('#blocked > table > tbody').append(
          '<tr rel="' +
            ...
            '">' +
            truncate(blockedarr[i][4]) +
            '</td><td class="text-right fpchoices" data-domain="' +
            itemdomain +
            '" rel="' +
            blockedarr[i][3] +
            '">' +
            clearBtn +
            '<span class="box box1 x_whitelist" rel="0" title="Allow Domain">' +
            bkg.getLocale('allow') +
        ...
        $('#blocked > table > tbody').append(
          '<tr rel="' +
            ...
            '</span>' +
            unwantedType +
            '<span class="box box3 x_bypass" rel="2" title="Temporary">' +
            bkg.getLocale('temp') +
            '</span></td>',
        );
      }
  ...
  if (allowedarrcount) {
    ...
    for (var i = 0; i < allowedarrcount; i++) {
      ...
      if (
        allowedarr[i][5] == '0' ||
        (allowedarr[i][7] && allowedarr[i][5] == '1')
      )
        clearBtn =
          '<span class="box box4" title="Clear Domain from List">' +
          bkg.getLocale('clear') +
      ...
      if (allowedarr[i][7]) {
        $('#allowed > table > tbody').append(
          '<tr rel="' +
            ...
            padZeros(entryTime.getSeconds()) +
            '</td><td title="' +
            allowedarr[i][1].replace(/"/g, "'") +
            '">' +
            truncate(allowedarr[i][1]) +
            '</td><td>' +
            allowedarr[i][2] +
            '</td><td title="' +
            allowedarr[i][4] +
            '">' +
            truncate(allowedarr[i][4]) +
            '</td><td class="text-right fpchoices" data-domain="' +
            itemdomain +
            '" rel="' +
            allowedarr[i][3] +
        ...
        $('#allowed > table > tbody').append(
          '<tr rel="' +
            ...
            '">' +
            clearBtn +
            '<span class="box box2 x_blacklist" rel="1" title="Deny">' +
            bkg.getLocale('deny') +
  ...
  if ($(this).parent().hasClass('fpchoices')) {
    ...
    if (clear) {
      ...
      if (val < 2) {
        ...
        $(this).addClass('selected');
        $(
          '#' + listType + " .choices[rel='" + url + "'] .x_whitelist",
        ).addClass('selected');
      } else if (val == 2) {
        if (selected) {
          ...
          $(this).removeClass('selected');
          $(
            '#' + listType + " .choices[rel='" + url + "'] .x_bypass",
          ).removeClass('selected');
        } else {
          var mode;
