...
import { updateDynamicRules } from '../common/rules';
...
var requestTypes: chrome.webRequest.OnBeforeRequestDetails['type'][],
  synctimer: ReturnType<typeof setTimeout>,
  recentstimer: ReturnType<typeof setTimeout>,
  reenabletimer: ReturnType<typeof setTimeout>,
  useragentinterval: ReturnType<typeof setInterval>,
  blackList: string[],
  whiteList: string[],
  distrustList: string[],
  trustList: string[],
...
var fpLists: Partial<Record<FingerprintType, string[]>> = {};

var fpListsSession: Partial<Record<FingerprintType, string[]>> = {};

var popup: [string, number] | [] = [];

var recentlog: RecentLog = {
  allowed: [],
  blocked: [],
};

var changed = false;
var ITEMS: Record<number, ItemsEntry> = {};
var experimental: NumericBool = 0;
...
export function refreshRequestTypes() {
  ...
  if (localStorage['script'] == 'true') requestTypes.push('script');
...
export function testWebRTC(rtcstatus: boolean) {
...
function checkWebRTC() {
  ...
  var doc = (document.getElementById('webrtc') as HTMLIFrameElement)
...
function mitigate(
  ...
  if (
    ...
    (localStorage['useragentspoof'] == 'off' &&
      localStorage['cookies'] == 'false' &&
      localStorage['referrerspoof'] == 'off')
  ) {
    return;
  }
  for (var i = 0, forcount = req.requestHeaders.length; i < forcount; i++) {
    if (
      req.requestHeaders[i].name == 'User-Agent' ||
      req.requestHeaders[i].name == 'Referer' ||
      req.requestHeaders[i].name == 'Cookie'
    ) {
      switch (req.requestHeaders[i].name) {
        ...
        case 'Referer':
          if (
            localStorage['referrerspoof'] != 'off' &&
            (localStorage['referrerspoofdenywhitelisted'] == 'true' ||
            ...
            else req.requestHeaders[i].value = localStorage['referrerspoof'];
          }
          break;
        case 'User-Agent':
          if (
            localStorage['useragentspoof'] != 'off' &&
            (localStorage['uaspoofallow'] == 'true' ||
              enabled(req.url) == 'true')
          ) {
            if (!userAgent || localStorage['useragentinterval'] == 'request')
              genUserAgent();
...
function genUserAgent(force?: NumericBool) {
  var os;
  if (localStorage['useragentspoof'] == 'custom') {
    ...
    if (userAgents) {
      ...
      if (uaCount == 1) userAgent = userAgents[0];
        ...
        if (localStorage['useragentinterval'] == 'off')
          userAgent = userAgents[0]; // use only first user agent string if set to off
        else {
          if (localStorage['useragentinterval'] == 'interval') {
            useragentinterval = window.setInterval(
              function () {
                genUserAgent(1);
              },
    ...
    else if (localStorage['useragentspoof_os'] == 'w7')
      os = 'Windows; U; Windows NT 6.1';
    else if (localStorage['useragentspoof_os'] == 'wv')
      os = 'Windows; U; Windows NT 6.0';
    else if (localStorage['useragentspoof_os'] == 'w2k3')
      os = 'Windows; U; Windows NT 5.2';
    else if (localStorage['useragentspoof_os'] == 'wxp')
      os = 'Windows; U; Windows NT 5.1';
    else if (localStorage['useragentspoof_os'] == 'w98')
      os = 'Windows; U; Windows 98';
    else if (localStorage['useragentspoof_os'] == 'w95')
      os = 'Windows; U; Windows 95';
    else if (localStorage['useragentspoof_os'] == 'linux64')
      os = 'X11; U; Linux x86_64';
    else if (localStorage['useragentspoof_os'] == 'linux32')
      os = 'X11; U; Linux x86_32';
    else if (localStorage['useragentspoof_os'] == 'machighsierra')
      os = 'Macintosh; U; Intel Mac OS X 10_13';
    else if (localStorage['useragentspoof_os'] == 'macsierra')
      os = 'Macintosh; U; Intel Mac OS X 10_12_2';
    else if (localStorage['useragentspoof_os'] == 'macelcapitan')
      os = 'Macintosh; U; Intel Mac OS X 10_11_6';
    else if (localStorage['useragentspoof_os'] == 'macyosemite')
      os = 'Macintosh; U; Intel Mac OS X 10_10_5';
    else if (localStorage['useragentspoof_os'] == 'macmavericks')
      os = 'Macintosh; U; Intel Mac OS X 10_9_5';
    else if (localStorage['useragentspoof_os'] == 'macmountainlion')
      os = 'Macintosh; U; Intel Mac OS X 10_8_5';
    else if (localStorage['useragentspoof_os'] == 'maclion')
      os = 'Macintosh; U; Intel Mac OS X 10_7_5';
    else if (localStorage['useragentspoof_os'] == 'macsnow')
      os = 'Macintosh; U; Intel Mac OS X 10_6_8';
    else if (localStorage['useragentspoof_os'] == 'freebsd64')
      os = 'X11; U; FreeBSD amd64';
    else if (localStorage['useragentspoof_os'] == 'freebsd32')
      os = 'X11; U; FreeBSD i686';
    else if (localStorage['useragentspoof_os'] == 'netbsd64')
      os = 'X11; U; NetBSD amd64';
    else if (localStorage['useragentspoof_os'] == 'netbsd32')
      os = 'X11; U; NetBSD i686';
    else if (localStorage['useragentspoof_os'] == 'openbsd64')
      os = 'X11; U; OpenBSD i686';
    else if (localStorage['useragentspoof_os'] == 'openbsd32')
      os = 'X11; U; OpenBSD i686';
    else if (localStorage['useragentspoof_os'] == 'chromeos')
      os = 'X11; U; CrOS i686 0.13.507';
    if (localStorage['useragentspoof'] == 'chrome63')
      ...
      userAgent =
        'Mozilla/5.0 (' +
        os +
        ') AppleWebKit/537.36 (KHTML, like Gecko) Chrome/50.0.2661.94 Safari/537.36 OPR/37.0.2178.43';
    else if (localStorage['useragentspoof'] == 'chrome14')
      userAgent =
        'Mozilla/5.0 (' +
        os +
        ') AppleWebKit/535.1 (KHTML, like Gecko) Chrome/14.0.835.94 Safari/535.1';
    else if (localStorage['useragentspoof'] == 'chrome13')
      userAgent =
        'Mozilla/5.0 (' +
        os +
        ') AppleWebKit/535.1 (KHTML, like Gecko) Chrome/13.0.782.43 Safari/535.1';
    else if (localStorage['useragentspoof'] == 'chrome12')
      userAgent =
        'Mozilla/5.0 (' +
        os +
        ') AppleWebKit/534.30 (KHTML, like Gecko) Chrome/12.0.750.0 Safari/534.30';
    else if (localStorage['useragentspoof'] == 'opera49')
      userAgent =
        'Mozilla/5.0 (' +
        os +
        ') AppleWebKit/537.36 (KHTML, like Gecko) Chrome/60.0.3112.78 Safari/537.36 OPR/47.0.2631.39';
    else if (localStorage['useragentspoof'] == 'opera42')
      userAgent =
        'Mozilla/5.0 (' +
        os +
        ') AppleWebKit/537.36 (KHTML, like Gecko) Chrome/55.0.2883.75 Safari/537.36 OPR/42.0.2393.85';
      ...
      userAgent =
        'Mozilla/5.0 (' + os + '; rv:57.0) Gecko/20100101 Firefox/57.0';
    else if (localStorage['useragentspoof'] == 'firefox50')
      userAgent =
        'Mozilla/5.0 (' + os + '; rv:50.0) Gecko/20100101 Firefox/50.0';
    else if (localStorage['useragentspoof'] == 'firefox48')
      userAgent =
        'Mozilla/5.0 (' + os + '; rv:48.0) Gecko/20100101 Firefox/48.0';
    else if (localStorage['useragentspoof'] == 'firefox46')
      userAgent =
        'Mozilla/5.0 (' + os + '; rv:44.0) Gecko/20100101 Firefox/44.0';
    else if (localStorage['useragentspoof'] == 'firefox6')
      userAgent =
        'Mozilla/5.0 (' + os + '; rv:6.0a2) Gecko/20110613 Firefox/6.0a2';
      ...
      userAgent =
        'Mozilla/5.0 (' + os + '; rv:2.0.1) Gecko/20110606 Firefox/4.0.1';
    else if (localStorage['useragentspoof'] == 'firefox3')
      userAgent =
        'Mozilla/5.0 (' + os + '; rv:1.9.2.9) Gecko/20100913 Firefox/3.6.9';
    else if (localStorage['useragentspoof'] == 'edge')
      userAgent =
        'Mozilla/5.0 (' +
        os +
        ') AppleWebKit/537.36 (KHTML, like Gecko) 42.0.2311.135 Safari/537.36 Edge/12.246';
      ...
      userAgent =
        'Mozilla/5.0 (compatible; MSIE 10.0; ' + os + '; Trident/6.0)';
      ...
      userAgent =
        'Mozilla/5.0 (' + os + '; rv:25.6) Gecko/20150723 PaleMoon/25.6.0';
    else if (localStorage['useragentspoof'] == 'palemoon25')
      userAgent =
        'Mozilla/5.0 (' + os + '; rv:25.1) Gecko/20130308 PaleMoon/25.1';
    else if (localStorage['useragentspoof'] == 'vivaldi111')
      userAgent =
        'Mozilla/5.0 (' +
        os +
        ') AppleWebKit/537.36 (KHTML, like Gecko) Chrome/60.0.3112.91 Safari/537.36 Vivaldi/1.92.917.35';
    else if (localStorage['useragentspoof'] == 'vivaldi')
      userAgent =
        'Mozilla/5.0 (' +
        os +
        ') AppleWebKit/537.36 (KHTML, like Gecko) Chrome/40.0.2214.89 Safari/537.36 Vivaldi/1.0.83.38';
    else if (localStorage['useragentspoof'] == 'midori')
      userAgent =
        'Mozilla/5.0 (' +
        os +
        ') AppleWebKit/538.15 (KHTML, like Gecko) Chrome/18.0.1025.133 Safari/538.15 Midori/0.5';
    else if (localStorage['useragentspoof'] == 'qupzilla')
      userAgent =
        'Mozilla/5.0 (' +
        os +
        ') AppleWebKit/533.3 (KHTML, like Gecko) Qupzilla/1.1.5';
  }
}

function removeParams(str: string) {
  return str.replace(/#[^#]*$/, '').replace(/\?[^\?]*$/, '');
}

function UrlInList(
  url: string,
  elems: ItemsEntry['allowed'] | ItemsEntry['blocked'],
) {
  // thanks vnagarnaik!
  var foundElem = false;
...
function inlineblock(
  req: chrome.webRequest.OnHeadersReceivedDetails,
): chrome.webRequest.BlockingResponse | undefined {
  if (
    req.tabId == -1 ||
    req.url === 'undefined' ||
...
function ScriptSafe(
  ...
  var reqtype: WebRequestType = req.type;
  ...
  if (
    ...
    if (
      ...
      (localStorage['preservesamedomain'] == 'strict' &&
        extractedDomain == extractedReqDomain)
    )
  ...
  if (
    ...
    (reqtype == 'object' &&
      (localStorage['object'] == 'true' || localStorage['embed'] == 'true')) ||
  ...
  if (
    ...
    if (!UrlInList(cleanedUrl, ITEMS[req.tabId]['blocked'])) {
      ...
      ITEMS[req.tabId]['blocked'].push([
        cleanedUrl,
        reqtype.toUpperCase() as WebRequestTypeUpper,
        extractedReqDomain,
        domainCheckStatus,
...
function updateRecents(list: RecentList) {
...
function setRecents(list: RecentList) {
  var recentLimit = 25;
  var recentsLength = recentlog[list].length;
  if (recentsLength > recentLimit)
    recentlog[list] = recentlog[list].slice(recentsLength - recentLimit) as [];
}

export function getRecents(list: RecentList) {
  setRecents(list);
    ...
    if (paramstart != -1) {
      if (url.indexOf('utm_') > paramstart) {
        sanitized = sanitized.replace(
          /[\?\&]utm_(?:cid|reader|term|content|source|medium|campaign|name)=[^&#]+/gi,
          '',
        );
        if (sanitized.charAt(paramstart) == '&')
          sanitized =
            sanitized.substring(0, paramstart) +
            '?' +
            sanitized.substring(paramstart + 1);
      }
    }
    sanitized = sanitized.replace(
      /#utm_(?:cid|reader|term|content|source|medium|campaign)=.+/i,
      '',
    );
...
function hashTrackingClean(url: string) {
  if (
    localStorage['hashchecking'] == 'true' &&
    (localStorage['hashallow'] == 'true' || enabled(url) == 'true')
  ) {
...
function enabled(url: string): StringBool {
...
function enabledfp(
  ...
  if (
    ...
    (localStorage['clientrects'] == 'false' &&
      fptype == 'fpClientRectangles') ||
...
export function domainSort(
  hosts: AllowedEntry[] | BlockedEntry[] | string[],
): HostsList | string[] {
  if (hosts.length > 0) {
    if (typeof hosts[0] === 'object') {
      const objectHosts = hosts as AllowedEntry[] | BlockedEntry[];

      const sortedHosts: HostsList = [];

      const splitHosts: SplitHostsEntry[] = [];
      ...
      const stringHosts = hosts as string[];

      const sortedHosts: string[] = [];

      const splitHosts: [string, string][] = [];
...
export function trustCheck(domain: string) {
  if (in_array(domain, trustList)) return 1;
  if (in_array(domain, distrustList)) return 2;
  return false;
}

export async function topHandler(
  domain: string,
  mode: NumericBool | FingerprintType,
) {
  if (domain) {
    if (
      !domain.match(
...
function haystackSearch(needle: string, haystack: string[]) {
  var keys = [];
...
export async function domainHandler(
  ...
  listtype: EnumListType = EnumListType.Permanent,
) {
  if (domain) {
    action =
      typeof action === 'number' ? action : (parseInt(action) as HandlerAction);
    ...
    if (listtype == 0) {
      ...
      var tempWhitelist: string[] = JSON.parse(localStorage['whiteList']);
      var tempBlacklist: string[] = JSON.parse(localStorage['blackList']);
    } else if (listtype == 1) {
      ...
      var tempWhitelist: string[] = JSON.parse(sessionStorage['whiteList']);
      var tempBlacklist: string[] = JSON.parse(sessionStorage['blackList']);
    }
      ...
      if (domain.substr(0, 3) == '**.') {
        ...
        if (whiteInstancesCount || blackInstancesCount) {
          ...
          if (
            confirm(
              'ScriptSafe detected ' +
                (whiteInstancesCount + blackInstancesCount) +
                ' existing rule(s) for ' +
                tempDomain +
                ' (' +
                whiteInstancesCount +
            ...
            if (blackInstancesCount) {
              for (var x = 0; x < blackInstancesCount; x++) {
                tempBlacklist.splice(
                  tempBlacklist.indexOf(blackInstances[x]),
                  1,
                );
              }
            }
...
export function fpDomainHandler(
  domain: string,
  listtype: FingerprintType,
  action: HandlerAction,
  temp: EnumListType = EnumListType.Permanent,
) {
  if (domain) {
    action =
      typeof action === 'number' ? action : (parseInt(action) as HandlerAction);
    ...
    if (temp == 0) {
      if (typeof localStorage[listtype] === 'undefined')
        localStorage[listtype] = JSON.stringify([]);
      var tempList = JSON.parse(localStorage[listtype]);
    } else if (temp == 1) {
      if (typeof localStorage[listtype] === 'undefined')
        sessionStorage[listtype] = JSON.stringify([]);
          ...
          if (
            confirm(
              'ScriptSafe detected ' +
                ...
                ' existing rule(s) for ' +
                tempDomain +
                '.\r\nDo you want to delete them before trusting the entire ' +
                tempDomain +
                ' domain in order to avoid conflicts?\r\nNote: this might not necessarily remove all conflicting entries, particularly if they use regex (e.g. d?main.com).',
            )
          ) {
            ...
            if (
              !confirm(
                'Do you still want to proceed trusting the entire ' +
                  tempDomain +
                  ' domain?',
              )
            ) {
...
function optionExists(opt: OptionName) {
  return typeof localStorage[opt] !== 'undefined';
}

function defaultOptionValue(opt: OptionName, val: unknown) {
  if (!optionExists(opt)) localStorage[opt] = val;
}

export function setDefaultOptions(force: 1 | 2 = undefined) {
  ...
  if (force) {
    for (var i of Object.keys(settingNames) as Setting[]) {
      localStorage[i] = settingNames[i] as never;
    }
    updated = true;
  } else {
    for (var i of Object.keys(settingNames) as Setting[]) {
...
function resetTabData(id: number, url: string) {
  if (id && url) {
    ITEMS[id] = [0];
    ITEMS[id]['url'] = url;
    ITEMS[id]['blocked'] = [];
...
export function statuschanger(duration: number) {
  ...
  if (localStorage['enable'] == 'true') {
    ...
    if (duration) {
      ...
      reenabletimer = setTimeout(function () {
        localStorage['enable'] = 'true';
      }, duration);
    }
  } else {
...
async function tempHandler(request: TempRequest) {
...
async function removeTempHandler(request: RemoveTempRequest) {
  if (typeof request.url === 'object') {
    for (var i = 0, forcount = request.url.length; i < forcount; i++) {
...
export function checkTemp(domain: string) {
...
chrome.tabs.onUpdated.addListener(function (tabid, changeinfo, tab) {
  if (localStorage['enable'] == 'true') {
    if (changeinfo.status == 'loading') {
      ...
      if (enabled(tab.url) == 'true') icontype = 'Forbidden';
      var extractedDomain = extractDomainFromURL(tab.url);
      if (
        in_array(extractedDomain, sessionWhiteList) ||
        in_array(extractedDomain, sessionBlackList)
      )
        icontype = 'Temp';
      ...
      if (typeof ITEMS[tabid] !== 'undefined') {
        ...
        if (
          localStorage['mode'] == 'block' &&
          typeof ITEMS[tabid]['allowed'] !== 'undefined'
        ) {
          for (
            var i = 0, forcount = ITEMS[tabid]['allowed'].length;
            i < forcount;
            i++
          ) {
            if (
              in_array(
                extractDomainFromURL(ITEMS[tabid]['allowed'][i][0]),
                sessionWhiteList,
              )
            ) {
              chrome.browserAction.setIcon({
          ...
          localStorage['mode'] == 'allow' &&
          typeof ITEMS[tabid]['blocked'] !== 'undefined'
        ) {
          for (
            var i = 0, forcount = ITEMS[tabid]['blocked'].length;
            i < forcount;
            i++
          ) {
            if (
              in_array(
                extractDomainFromURL(ITEMS[tabid]['blocked'][i][0]),
                sessionBlackList,
              )
            ) {
              chrome.browserAction.setIcon({
  ...
  port.onDisconnect.addListener(function () {
    if (popup.length > 0) {
      if (localStorage['refresh'] == 'true')
        chrome.tabs.update(popup[1], { url: popup[0] });
      popup = [];
    }
  });
});

chrome.runtime.onMessage.addListener(async function <T extends MessageRequest>(
  request: T,
  sender: chrome.runtime.MessageSender,
  sendResponse: (response: never) => void,
) {
  if (request.reqtype == 'get-settings') {
    var fpListStatus: Partial<Record<FingerprintType, FingerprintEnabledType>> =
      {};

    var extractedDomain = extractDomainFromURL(sender.tab.url);
    for (let i in fpTypes) {
      fpListStatus[fpTypes[i]] = enabledfp(extractedDomain, fpTypes[i]);
    }

    const send = sendResponse as SendResponseCallback<GetSettingsRequest>;
    ...
    var fptype: FingerprintDescription;
    var cleanedUrl = removeParams(sender.tab.url);
    for (let i of Object.keys(fpListStatus) as FingerprintType[]) {
      if (fpListStatus[i] != -1) {
    ...
    const send = sendResponse as SendResponseCallback<GetListRequest>;
    ...
    var enableval: EnableValue = domainCheck(request.url);
    ...
    for (let i of Object.keys(fpListsSession) as FingerprintType[]) {
    ...
    if (request.src) {
      ...
      if (!UrlInList(cleanedUrl, ITEMS[sender.tab.id]['allowed'])) {
        ...
        if (extractedDomain.substr(0, 4) == 'www.')
          extractedDomain = extractedDomain.substr(4);
        // var allowedDomainCheck = domainCheck(request.src, 1); // TODO: Remove
    ...
    const send = sendResponse as (response: {}) => void;
...
function genContextMenu() {
  ...
  if (localStorage['mode'] == 'block') {
    ...
    chrome.contextMenus.create({
      title: getLocale('blockallallowed'),
      parentId: parent,
      onclick: tempPage,
    });
    chrome.contextMenus.create({
  ...
  chrome.contextMenus.create({
    title: getLocale('revoketemp'),
    parentId: parent,
    onclick: removeTempPage,
  });
  chrome.contextMenus.create({
    title: getLocale('revoketempall'),
    parentId: parent,
    onclick: removeTempAll,
  });
  chrome.contextMenus.create({ parentId: parent, type: 'separator' });
  chrome.contextMenus.create({
...
async function tempPage() {
  ...
  const tempKey = (tempMode + 'ed') as ItemsEntryKey;
  ...
  ITEMS[tabs[0].id][tempKey].map(function (
    items: (ItemsEntry['allowed'] | ItemsEntry['blocked'])[0],
  ) {
...
async function bytesToBase64DataUrl(
  bytes: pako.Uint8ArrayReturnType,
...
async function dataUrlToBytes(
  dataUrl: string,
): Promise<pako.Uint8ArrayReturnType> {
  const res = await fetch(dataUrl);
  return new Uint8Array(await res.arrayBuffer());
}

async function ssCompress(str: string) {
  // @ts-ignore
  const deflated = pako.deflate(str);
  return await bytesToBase64DataUrl(deflated);
}

async function ssDecompress(str: string) {
  const data = await dataUrlToBytes(str);
  return pako.inflate(data, { to: 'string' });
}

export async function freshSync(force: boolean = undefined) {
  if (storageapi && localStorage['syncenable'] == 'true') {
    window.clearTimeout(synctimer);
    if (force) {
      localStorage['sync'] = 'true';
      var settingssync: Record<string, string | number> = {};
      var simplesettings = '';
      ...
      for (var k in localStorage) {
        if (localStorage.hasOwnProperty(k)) {
          ...
          if (
            ...
            k != 'blackListCount2' &&
            k != 'useragentCount2' &&
            k.substr(0, 10) != 'whiteList_' &&
            k.substr(0, 10) != 'blackList_' &&
            k.substr(0, 2) != 'zb' &&
            k.substr(0, 2) != 'zw' &&
            k.substr(0, 2) != 'sw' &&
            k.substr(0, 2) != 'sb' &&
      ...
      if (zarr['zw'].length) {
        for (var x = 0, forcount = zarr['zw'].length; x < forcount; x++)
          delete localStorage[zarr['zw'][x]];
      }
      if (zarr['sw'].length) {
        for (var x = 0, forcount = zarr['sw'].length; x < forcount; x++)
          delete localStorage[zarr['sw'][x]];
      }
      // legacy syncing method - start
      jsonstr = (JSON.parse(localStorage['whiteList']) as string[]).toString();
      i = 0;
      limit =
        chrome.storage.sync.QUOTA_BYTES_PER_ITEM -
        Math.ceil(
          jsonstr.length / (chrome.storage.sync.QUOTA_BYTES_PER_ITEM - 4),
        ) -
        4;
        ...
        chrome.storage.sync.clear(function () {
          chrome.storage.sync.set(settingssync, function () {
            if (chrome.extension.lastError) {
              ...
              if (localStorage['syncnotify'] == 'true')
                chrome.notifications.create(
                  'syncnotify',
                  {
                    type: 'basic',
                    iconUrl: '../img/icon48.png',
                    title: 'ScriptSafe - ' + getLocale('exportsuccesstitle'),
                    message: getLocale('exportsuccess'),
                  },
                  function () {
                    return true;
                  },
...
export function importSyncHandle(mode: NumericBool) {
  if (storageapi) {
    if (
      mode == 1 ||
              ...
              if (localStorage['syncfromnotify'] == 'true')
                chrome.notifications.create(
                  'syncnotify',
                  {
                    type: 'basic',
                    iconUrl: '../img/icon48.png',
                    title: 'ScriptSafe - ' + getLocale('importsuccesstitle'),
                    message: getLocale('importsuccess'),
                  },
                  function () {
                    updated = true;
                    return true;
...
async function importSync(changes: any) {
  ...
  if (optionExists('fpCount')) {
    ...
    for (var i = 0, forcount = localStorage['fpCount']; i < forcount; i++) {
      if (localStorage['sf' + i]) {
        if (localStorage['sf' + i].substr(0, 13) == localStorage['lastSync'])
...
async function listsSyncParse(type: ListType) {
  if (
    optionExists((type + 'Count') as ListTypeCount) ||
    optionExists((type + 'Count2') as ListTypeCount)
  ) {
    var lsName = type.substr(0, 1);
    var concatlist = '';
    var concatlistarr: string[] = [];
    var counttype;
    var listerror = false;

    if (optionExists((type + 'Count2') as ListTypeCount))
      ...
      for (var i = 0, forcount = localStorage[counttype]; i < forcount; i++) {
        if (counttype == type + 'Count2') {
          if (localStorage['s' + lsName + i]) {
            if (
              localStorage['s' + lsName + i].substr(0, 13) ==
              localStorage['lastSync']
            )
              concatlist += localStorage['s' + lsName + i].substr(13);
            else {
              listerror = true;
            }
    ...
    if (!listerror) {
      ...
      alert(
        'Incomplete ' +
          type.toLowerCase() +
          ' data was detected. Very large lists are known to cause issues with syncing.\r\nAs a safety precaution, your ' +
          type.toLowerCase() +
          ' has not been updated and syncing has been disabled on this device to prevent overwriting data on other devices.\r\nPlease consider manually exporting your latest settings and importing it into your other devices from the Options page.',
      );
      localStorage['syncenable'] = 'false';
    }
    ...
    if (optionExists((type + 'Count') as ListTypeCount))
...
export function cacheLists() {
  var tempList: string[] = JSON.parse(localStorage['whiteList']);
  var tempDomain: string[] = [];
  var tempWildDomain: string[] = [];
...
export function cacheFpLists() {
  for (var i in fpTypes) {
    var tempList: string[] = JSON.parse(localStorage[fpTypes[i]]);
    var tempDomain: string[] = [];
    tempList.map(function (domain) {
      tempDomain.push(domain);
    });
...
export function getLocale(str: string): string {
  if (locale) {
...
var uiLang = chrome.i18n.getUILanguage().replace(/-/g, '_') as keyof Langs;

if (!optionExists('locale')) {
  localStorage['locale'] = 'en_US';
  if ((uiLang as string) != 'en' && uiLang != 'en_GB' && uiLang != 'en_US') {
    if (typeof langs[uiLang] !== 'undefined') {
      if (
        confirm(
          'ScriptSafe detected that your browser is currently set to ' +
            langs[uiLang] +
            '.\r\nWould you like to use ScriptSafe in ' +
            langs[uiLang] +
  ...
  if (!optionExists('version') || localStorage['version'] != version) {
    ...
    if (!optionExists('tempregexflag')) {
      if (optionExists('version')) {
        var tempList: string[] = JSON.parse(localStorage['blackList']);
        var tempNewList: string[] = [];
        if (tempList.length) {
          tempList.map(function (domain) {
  ...
  if (typeof chrome.webRequest !== 'undefined') {
    ...
    if (typeof chrome.webRequest !== 'undefined') {
      chrome.webRequest.onBeforeRequest.addListener(
        ScriptSafe,
        { types: requestTypes, urls: requestUrls },
        ['blocking'],
      );
