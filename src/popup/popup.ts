...
import 'bootstrap';
import '../common/glyph.css';
import './popup.scss';

var version = '1.0.9.3';
var port = chrome.runtime.connect({ name: 'popuplifeline' });
var bkg = chrome.extension.getBackgroundPage() as BackgroundWindow;

var closepage: StringBool,
  mode: Mode,
  taburl: string,
  tabid: number,
  tabdomain: string;
...
var statuschange = function (this: HTMLElement) {
  $(this).hide();
  $(this).after(
    bkg.getLocale('disable') +
      ': <span class="box box3" data-duration="5">5m</span> <span class="box box3" data-duration="15">15m</span> <span class="box box3" data-duration="30">30m</span> <span class="box box3" data-duration="60">1h</span> <span class="box box2" data-duration="">' +
...
var _statuschanger = function (this: HTMLElement) {
  port.postMessage({ url: taburl, tid: tabid });
  bkg.statuschanger(parseInt($(this).attr('data-duration')));
  window.close();
};

var revokealltemp = function (this: HTMLElement) {
  port.postMessage({ url: taburl, tid: tabid });
  bkg.revokeTemp();
  window.close();
};

var bulkhandle = function (this: HTMLElement) {
  port.postMessage({ url: taburl, tid: tabid });
  bulk($(this));
};

var removehandle = async function (this: HTMLElement) {
  await remove(tabdomain, $(this), 0);
};

var x_removehandle = async function (this: HTMLElement) {
  await remove($(this).parent().attr('rel'), $(this), 1);
};

var savehandle = async function (this: HTMLElement) {
  port.postMessage({ url: taburl, tid: tabid });
  await save(tabdomain, $(this), 0);
};

var x_savehandle = async function (this: HTMLElement) {
  port.postMessage({ url: taburl, tid: tabid });
...
document.addEventListener('DOMContentLoaded', function () {
  setTimeout(init, 150);
  $('#pop_ay').mouseup(function (e) {
    if (e.which != 3) openTab('https://twitter.com/andryou');
  });
  $('#pop_docs').mouseup(function (e) {
    if (e.which != 3) openTab('https://www.andryou.com/scriptsafe/');
  });
  $('#pop_project').mouseup(function (e) {
    if (e.which != 3) openTab('https://github.com/andryou/scriptsafe');
  });
  $('#pop_options').mouseup(function (e) {
    if (e.which != 3) openTab(chrome.extension.getURL('html/options.html'));
  });
  $('#pop_log').mouseup(function (e) {
    if (e.which != 3) openTab(chrome.extension.getURL('html/recents.html'));
  });
  $('#pop_webstore').mouseup(function (e) {
    if (e.which != 3)
      openTab(
  ...
  $('#pop_close')
    ...
    .attr('title', bkg.getLocale('close'));
  $('#pop_refresh').mouseup(function (e) {
    if (e.which != 3) chrome.tabs.reload();
    window.close();
  });
});
  ...
  chrome.tabs.query({ active: true, currentWindow: true }, function (_tab) {
    ...
    if (
      ...
      chrome.runtime.sendMessage(
        ...
        function (response: GetListResponse) {
          if (typeof response === 'undefined' || response == 'reload') {
            if (tab.url.substring(0, 4) == 'http') {
              $('table').html(
                '<tr><td>' + bkg.getLocale('recentlyupdated') + '</td></tr>',
              );
            } else {
              $('table').html(
                '<tr><td>' + bkg.getLocale('cannotprocess') + '</td></tr>',
              );
            }
            return;
          }
          ...
          if (response.rating == 'true')
            $('#currentdomain').html(
              '<span class="domainname domainoutput"><span class="wot"><a href="http://www.mywot.com/en/scorecard/' +
                tabdomain +
                '" target="_blank" title="' +
                bkg.getLocale('ratingbtn') +
                ': ' +
                tabdomain +
            ...
            $('#currentdomain').html(
              '<span class="domainname domainoutput">' + tabdomain + '</span>',
            );
          if (
            ...
            if (response.status == 'false') {
              ...
              $('.thirds')
                ...
                .html('<i>' + bkg.getLocale('ssdisabled') + '</i>');
              $('#parent')
                .css('text-align', 'center')
                .append(
                  '<div class="box box1 snstatus" title="' +
            ...
            if (responseBlockedCount != 0) {
              ...
              $('.thirds')
                .parent()
                .after(
                  "<tr><td class='bolded' style='height: 14px; padding-top: 5px;'><span class='blocked'>" +
                    bkg.getLocale('blockeditems') +
              ...
              for (var i = 0; i < responseBlockedCount; i++) {
                ...
                if (itemdomain) {
                  ...
                  if (
                    ...
                    if (domainCheckStatus == 1) {
                      ...
                      $('#blocked').append(
                        '<div class="thirditem" title="[' +
                          ...
                          itemdomainfriendly +
                          '">1</span>)</span><br /><span class="choices" rel="' +
                          itemdomain +
                          '" sn_list="' +
                          allowedtype +
                          '"><span class="box box4 x_' +
                          itemdomainfriendly +
                          '" title="Clear Domain from List">' +
                          bkg.getLocale('clear') +
                          '</span><span class="box box1 x_whitelist" rel="0" title="Allow Domain">' +
                          bkg.getLocale('allow') +
                          '</span><span class="box box1 x_trust' +
                          trustval0 +
                          '" rel="3" title="Trust Entire Domain">' +
                          bkg.getLocale('trust') +
                          '</span><span class="box box2 x_blacklist selected" rel="1" title="Deny">' +
                          bkg.getLocale('deny') +
                          '</span><span class="box box2 x_trust' +
                          trustval1 +
                          '" rel="4" title="Distrust Entire Domain">' +
                          bkg.getLocale('distrust') +
                          '</span><span class="box box3 x_bypass" rel="2" title="Temporary">' +
                          bkg.getLocale('temp') +
                          '</span></span></div>',
                      );
                    } else {
                      if (
                        ...
                        if (
                          ...
                          $('#blocked').append(
                            '<div class="fpcat" rel="x_' +
                              ...
                              '"></span><span class="domainoutput">' +
                              fpitemdomain +
                              '</span> (<span rel="fpcount_' +
                              fpitemdomainfriendly +
                              '">1</span>)</span><br /><span class="choices fpchoices" rel="' +
                              fpitemdomain +
                              '" sn_list="' +
                              itemdomain +
                              '"><span style="display: none;" class="box box4 fp_' +
                              fpitemdomainfriendly +
                              '" title="Clear Domain from List">' +
                              bkg.getLocale('clear') +
                              '</span><span class="box box1 x_whitelist" rel="0" title="Allow Domain">' +
                        ...
                        $('#blocked').append(
                          '<div class="thirditem" title="[' +
                            ...
                            '">1</span>)</span><br /><span class="choices" rel="' +
                            itemdomain +
                            '" sn_list="-1"><span class="box box4 x_' +
                            itemdomainfriendly +
                            '" title="Clear Domain from List">' +
                            bkg.getLocale('clear') +
                            '</span><span class="box box1 x_whitelist" rel="0" title="Allow Domain">' +
                            bkg.getLocale('allow') +
                            '</span><span class="box box1 x_trust" rel="3" title="Trust Entire Domain">' +
                            bkg.getLocale('trust') +
                            '</span><span class="box box2 x_blacklist" rel="1" title="Deny">' +
                            bkg.getLocale('deny') +
                            '</span><span class="box box2 x_trust" rel="4" title="Distrust Entire Domain">' +
                            bkg.getLocale('distrust') +
                            '</span><span class="box box3 x_bypass" rel="2" title="Temporary">' +
                            bkg.getLocale('temp') +
                            '</span></span></div>',
                    ...
                    $("#blocked [rel='x_" + itemdomainfriendly + "']").attr(
                      'title',
                      $("#blocked [rel='x_" + itemdomainfriendly + "']").attr(
                        'title',
                      ) +
                  ...
                  if (response.rating == 'true') {
                    $("#blocked [rel='r_" + itemdomainfriendly + "']").html(
                      '<span class="wot"><a href="http://www.mywot.com/en/scorecard/' +
                        itemdomain.replace(/[\[\]]/g, '') +
                        '" target="_blank" title="' +
                        bkg.getLocale('ratingbtn') +
                        ': ' +
                        itemdomain +
                  ...
                  if (
                    ...
                    if (response.antisocial == 'true' && baddiesstatus == 2) {
                      $(
                        ...
                        .attr('title', 'Antisocial')
                        .html(bkg.getLocale('antisocialpopup'))
                        .addClass('selected');
                    } else {
                      $(
                        "#blocked [rel='x_" +
                        ...
                        .attr('title', 'Unwanted Content Provider')
                        .html(bkg.getLocale('unwanted'))
                        .addClass('selected');
                    }
                  } else if (
                    ...
                    $(
                      ...
                      .attr(
                        'title',
                        'Ignored allowed domain due to unlisted tab domain',
                      )
                      .html(bkg.getLocale('ignoredallow'))
                      .addClass('selected');
                  } else if (
                    response.annoyances == 'true' &&
                    domainCheckStatus == -1 &&
                    ...
                    $(
                      "#blocked [rel='x_" + itemdomainfriendly + "'] .x_trust",
                    ).hide();
                  }
                  if (mode == 'allow') {
              ...
              $('#blocked').append(
                $('.thirditem:has([title="Unwanted Content Provider"])'),
              );
              ...
              $('#blocked').append(
                $("#blocked [rel='x_bluetooth_enumeration']"),
              );
              $('#blocked').append($("#blocked [rel='x_client_rectangles']"));
              $('#blocked').append(
                $("#blocked [rel='x_clipboard_interference']"),
              );
              $('#blocked').append(
                $("#blocked [rel='x_browser_plugins_enumeration']"),
              );
              $('#blocked').prepend(
                $(
                  "#blocked [data-domain='" +
              ...
              $("#blocked [rel='fp_" + tabdomainfriendly + "']")
                .children()
                .css('font-weight', 'bold');
              $('#blocked').prepend(
                $("#blocked [rel='x_" + tabdomainfriendly + "']"),
            ...
            if (responseAllowedCount != 0) {
              ...
              $('.thirds')
                ...
                .parent()
                .append(
                  "<tr><td class='bolded' style='height: 14px;'><span class='allowed'>" +
                    bkg.getLocale('alloweditems') +
              ...
              for (var i = 0; i < responseAllowedCount; i++) {
                ...
                if (itemdomain) {
                  ...
                  if (
                    ...
                    if (allowedItems[i][3] == 0) {
                      ...
                      $('#allowed').append(
                        '<div class="thirditem" title="[' +
                          ...
                          '" title="Clear Domain from List">' +
                          bkg.getLocale('clear') +
                          '</span><span class="box box1 x_whitelist selected" rel="0" title="Allow Domain">' +
                          bkg.getLocale('allow') +
                          '</span><span class="box box1 x_trust' +
                          trustval0 +
                          '" rel="3" title="Trust Entire Domain">' +
                        ...
                        if (
                          ...
                          $('#allowed').append(
                            '<div class="fpcat" rel="x_' +
                              ...
                              '"></span><span class="domainoutput">' +
                              fpitemdomain +
                              '</span></span><br /><span class="choices fpchoices" rel="' +
                              fpitemdomain +
                              '" sn_list="' +
                              itemdomain +
                              '"><span style="display: none;" class="box box4 fp_' +
                    ...
                    $("#allowed [rel='x_" + itemdomainfriendly + "']").attr(
                      'title',
                      $("#allowed [rel='x_" + itemdomainfriendly + "']").attr(
                        'title',
                      ) +
                  ...
                  if (response.rating == 'true') {
                    $("#allowed [rel='r_" + itemdomainfriendly + "']").html(
                      '<span class="wot"><a href="http://www.mywot.com/en/scorecard/' +
                        itemdomain.replace(/[\[\]]/g, '') +
                        '" target="_blank" title="' +
                        bkg.getLocale('ratingbtn') +
                  ...
                  if (response.annoyances == 'true' && baddiesstatus == 1) {
                    ...
                    $(
                      "#allowed [rel='x_" + itemdomainfriendly + "'] .x_trust",
                    ).hide();
                  }
                  if (mode == 'block') {
                    if (bkg.in_array(itemdomain, response.temp)) {
              ...
              $("#allowed [rel='x_" + tabdomainfriendly + "']")
                .children()
                .first()
                .css('font-weight', 'bold');
              $("#allowed [rel='fp_" + tabdomainfriendly + "']")
                .children()
                .css('font-weight', 'bold');
              $('#allowed').prepend(
                $("#allowed [rel='x_" + tabdomainfriendly + "']"),
              ...
              $('#allowed').append(
                $("#allowed [rel='x_bluetooth_enumeration']"),
              );
              $('#allowed').append($("#allowed [rel='x_client_rectangles']"));
              $('#allowed').append(
                $("#allowed [rel='x_clipboard_interference']"),
              );
              $('#allowed').append(
                $("#allowed [rel='x_browser_plugins_enumeration']"),
              );
            }
            var blockedCount = blocked.length;
            var allowedCount = allowed.length;
            if (responseBlockedCount != 0 && blockedCount == 0)
              $('.thirds:first').html('<i>None</i>');
            if (responseAllowedCount != 0 && allowedCount == 0)
              $('.allowed').parent().hide();
            if (blockedCount != 0 && allowedCount != 0) {
              $('.allowed').parent().css('padding-top', '12px');
            }
            $('.x_whitelist,.x_blacklist,.x_trust,.x_bypass').bind(
              'click',
              x_savehandle,
            );
            var tempSel;
            ...
            if (mode == 'block') {
              if (
                ($('#blocked .thirditem').length == 1 &&
                  ($('#blocked .thirditem[rel="x_no_script"]').length == 1 ||
                    $('#blocked .thirditem[rel="x_web_bug"]').length == 1 ||
                    $('#blocked .thirditem[rel="x_canvas_fingerprint"]')
                      .length == 1 ||
                    $('#blocked .thirditem[rel="x_canvas_font_access"]')
                      .length == 1 ||
                    $('#blocked .thirditem[rel="x_audio_fingerprint"]')
                      .length == 1 ||
                    $('#blocked .thirditem[rel="x_webgl_fingerprint"]')
                      .length == 1 ||
                    $('#blocked .thirditem[rel="x_battery_fingerprint"]')
                      .length == 1 ||
                    $('#blocked .thirditem[rel="x_device_enumeration"]')
                      .length == 1 ||
                    $('#blocked .thirditem[rel="x_gamepad_enumeration"]')
                      .length == 1 ||
                    $('#blocked .thirditem[rel="x_webvr_enumeration"]')
                      .length == 1 ||
                    $('#blocked .thirditem[rel="x_bluetooth_enumeration"]')
                      .length == 1)) ||
                $('#blocked .thirditem[rel="x_spoofed_timezone"]').length ==
                  1 ||
                $('#blocked .thirditem[rel="x_client_rectangles"]').length ==
                  1 ||
                $('#blocked .thirditem[rel="x_clipboard_interference"]')
                  .length == 1 ||
                $('#blocked .thirditem[rel="x_data_url"]').length == 1 ||
                $('#blocked .thirditem[rel="x_browser_plugins_enumeration"]')
                  .length == 1
              ) {
                // empty space
              } else {
                if ($('#blocked .x_whitelist:visible').length != 0) {
                  $(tempSel).append(
                    '<br /><div class="box box3 allowsession" title="Allow all blocked items for the session (not including webbugs/noscript/fingerprinting/annoyances)">' +
                      bkg.getLocale('allowallblocked') +
                      '</div>',
                  );
                } else {
                  $(tempSel).append('<br />');
              ...
              $(tempSel).append(
                '<br /><div class="box box3 allowsession" title="Block all allowed items for the session">' +
                  bkg.getLocale('blockallallowed') +
                  '</div>',
              );
            }
            ...
            if (intemp || tabInTemp) {
              ...
              $(tempSel).append(
                '<div class="box box5 prevoke" title="Revoke temporary permissions given to the current page">' +
                  bkg.getLocale('revoketemp') +
                  '</div>',
              );
          ...
          if (
            (typeof response.temp !== 'undefined' && response.temp.length) ||
            (typeof response.tempfp !== 'undefined' && response.tempfp)
          ) {
            if ($(tempSel + ' > br').length == 0) $(tempSel).append('<br />');
            $(tempSel).append(
              '<div class="box box5 clearglobaltemp" title="Revoke all temporary permissions given in this entire browsing session">' +
                bkg.getLocale('revoketempall') +
                '</div>',
            );
            $('.clearglobaltemp').bind('click', revokealltemp);
          } else if (
            $(tempSel + ' > br').length == 1 &&
            $(tempSel + ' > div.allowsession').length == 0
          )
            $(tempSel + ' > br').remove();
          $('#parent')
            .prepend(
              '<div class="box box4 pclear" title="Clear Domain from List">' +
                bkg.getLocale('clear') +
                '</div><div class="box box1 pallow" rel="0" title="Allow Current Domain">' +
                bkg.getLocale('allow') +
                '</div><div class="box box1 ptrust" rel="3" title="Trust Entire Domain">' +
                bkg.getLocale('trust') +
                '</div><div class="box box2 pdeny" rel="1" title="Deny">' +
                bkg.getLocale('deny') +
                '</div><div class="box box2 ptrust" rel="4" title="Distrust Entire Domain">' +
                bkg.getLocale('distrust') +
                '</div><div class="box box3 pbypass" rel="2" title="Temporary">' +
                bkg.getLocale('temp') +
                '</div>',
            )
          ...
          if (response.enable == 1 || response.enable == 4) {
            if (tabInTemp) {
              ...
              $("#blocked [rel='x_" + tabdomainfriendly + "'] .x_blacklist")
                .removeClass('selected')
                .bind('click', x_savehandle);
            ...
            if (
              ...
              if (response.antisocial == 'true' && baddiesStatus == 2) {
                $('.pdeny')
                  ...
                  .text(bkg.getLocale('antisocialpopup'));
              } else {
                $('.pdeny')
                  .addClass('selected')
                  .attr('title', 'Blocked (provider of unwanted content)')
                  .text(bkg.getLocale('blocked'));
              }
              $(
                ".pbypass, .ptrust[rel='3'], .ptrust[rel='4'], .pclear, .pallow",
              ).hide();
            } else if (
              response.annoyances == 'true' &&
              domainCheckStatus == -1 &&
            ...
            if (tabInTemp) {
              ...
              $("#allowed [rel='x_" + tabdomainfriendly + "'] .x_whitelist")
                .removeClass('selected')
                .bind('click', x_savehandle);
          ...
          if (response.status == 'true')
            $('#credit').append(
              '&nbsp;|&nbsp;<span class="box box2 snstatus" title="Disable ScriptSafe">' +
                bkg.getLocale('disable') +
                '</span>',
            );
          ...
          $('.fphead').bind('click', function () {
            $(this).next().toggle();
            if ($('.chevron', this).hasClass('uparrow'))
              $('.chevron', this).removeClass('uparrow');
...
async function remove(url: string, el: JQuery, type: NumericBool) {
  var val = el.attr('rel');
  ...
  if (el.parent().hasClass('fpchoices')) {
    ...
    else if (fpType == 'browser.plugins.enumeration')
  ...
  if (closepage == 'true') window.close();
    ...
    if (
      el.parent().attr('sn_list') == '0' ||
      el.parent().attr('sn_list') == '3'
    ) {
      $(".thirditem[rel='x_" + urlfriendly + "'] .choices, #parent").attr(
        'sn_list',
        '-1',
      );
    }
    ...
    if (type == 0) {
      ...
      if (
        $(".thirditem[rel='x_" + urlfriendly + "'] .x_blacklist").text() ==
        'Unwanted'
      )
        $(".thirditem[rel='x_" + urlfriendly + "'] .x_blacklist").addClass(
          'selected',
        );
      $('.pbypass').show();
      ...
      if ($('.x_blacklist', el.parent()).text() == 'Unwanted')
        $('.x_blacklist', el.parent()).addClass('selected');
    }
  }
}

async function save(url: string, el: JQuery, type: NumericBool) {
  ...
  if (closepage == 'true') window.close();
    ...
    if (type == 0) {
      ...
      $('.thirditem .x_' + urlfriendly).hide();
      if (val == 0)
        $(".thirditem[rel='x_" + urlfriendly + "'] .x_whitelist").addClass(
          'selected',
        );
      else if (val == 1)
        $(".thirditem[rel='x_" + urlfriendly + "'] .x_blacklist").addClass(
        ...
        $(".thirditem[rel='x_" + urlfriendly + "'] .x_bypass").addClass(
          'selected',
        );
      $('.pclear').hide();
      if (el.attr('rel') == '3') {
        $(
          ".pallow, .thirditem[rel='x_" + urlfriendly + "'] .x_trust[rel='3']",
        ).addClass('selected');
      } else if (el.attr('rel') == '4') {
        $(
          ".pdeny, .thirditem[rel='x_" + urlfriendly + "'] .x_trust[rel='4']",
        ).addClass('selected');
      }
      if (val < 2) {
        ...
        if (!selected) {
          ...
          $(".thirditem[rel='x_" + urlfriendly + "'] .x_bypass").removeClass(
            'selected',
          );
        }
      }
      ...
      if (val < 2) {
        ...
        if (!selected) {
          el.addClass('selected');
          if (url == tabdomain && !el.parent().hasClass('fpchoices'))
            $('.pbypass').addClass('selected').show();
        } else {
          if (url == tabdomain && !el.parent().hasClass('fpchoices'))
            $('.pbypass').removeClass('selected').show();
          if ($('.x_blacklist', el.parent()).text() == 'Unwanted')
            $('.x_blacklist', el.parent()).addClass('selected');
        }
        $('.x_bypass', el.parent()).show();
